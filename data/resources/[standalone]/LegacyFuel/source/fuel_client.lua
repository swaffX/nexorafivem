...
RegisterCommand("setfuel", function(source, args)
    ...
    -- Eğer oyuncu araçta değilse, son bindiği aracı bul
    if vehicle == 0 then
    ...
    if not fuelLevel or fuelLevel < 0 or fuelLevel > 100 then
        QBCore.Functions.Notify("Lütfen 0 ile 100 arasında bir değer girin.", "error")
        return
    end

    -- Yakıt ayarla
    SetFuel(vehicle, fuelLevel)
    QBCore.Functions.Notify("Yakıt seviyesi " .. fuelLevel .. "% olarak ayarlandı.", "success")
end, false)
