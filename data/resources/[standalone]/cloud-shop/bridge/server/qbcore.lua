...
local function canCarryItem(source, itemName, quantity)
	...
	elseif GetResourceState("qb-inventory") == "started" then
		if not doesExportExist("qb-inventory", "CanAddItem") then
			log.error("[Bridge.Item.CanAdd] Could not find [qb-inventory:CanAddItem] export")
			log.warn("[Bridge.Item.CanAdd] Please update your qb-inventory version to 2.0.0 or higher to use this export")
			return true
		end

		local success, message = exports["qb-inventory"]:CanAddItem(source, itemName, quantity)
		if message then log.debug("CanAddItem: " .. message) end
...
local function addItem(source, itemName, quantity)
	...
	elseif GetResourceState("qb-inventory") == "started" then
		if isWeapon then return exports["qb-inventory"]:AddItem(source, itemName, quantity, false, { quality = 100 }, "cloud-shop:addWeapon"), "Item added" end
		return exports["qb-inventory"]:AddItem(source, itemName, quantity, false, false, "cloud-shop:addItem"), "Item added"
	else
