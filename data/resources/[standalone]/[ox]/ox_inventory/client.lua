...
local function canOpenInventory()
	...
	--and not exports["liverp-base"]:KelepceKontrol()
	and invOpen ~= nil
	...
	if canUseItem(data.ammo and true) then
		...
		elseif currentWeapon then
			if data.ammo then
				...
				print(maxAmmo, clipSize, currentAmmo)
				...
				useItem(data, function(resp)
					...
					if cache.vehicle then
						SetAmmoInClip(playerPed, currentWeapon.hash, clipSize)

						if cache.seat > -1 or IsVehicleStopped(cache.vehicle) then
							TaskReloadWeapon(playerPed, true)
						end
					else
						AddAmmoToPed(playerPed, currentWeapon.hash, clipSize)
						Wait(100)
						TaskReloadWeapon(playerPed, true)
					...
					local retval, clipMagazineCount = GetAmmoInClip(playerPed, currentWeapon.hash)
					local clipCount = retval and clipMagazineCount or 0
					lib.callback.await('ox_inventory:updateWeapon', false, 'load', (currentAmmo + clipCount + clipSize), false, currentWeapon.metadata.specialAmmo)
				end)
...
local function registerCommands()
	...
	local primary = lib.addKeybind({
		...
		onPressed = function()
			...
			if not entity or entityType ~= 2 or not Inventory.OpenTrunk(entity) then
