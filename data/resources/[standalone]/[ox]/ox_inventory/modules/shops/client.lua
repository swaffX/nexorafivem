...
for shopType, shopData in pairs(data('shops') --[[@as table<string, OxShop>]]) do
	...
	if shared.target then
		...
		shop.model = shopData.model or nil	
...
local function SpawnPed(ped, coords)
	local model = lib.requestModel(ped)

	if not model then return end

	local entity = CreatePed(0, model, coords.x, coords.y, coords.z, coords.heading, false, true)
	...
	for type, shop in pairs(shopTypes) do
		...
		elseif shop.locations then
			...
			for i = 1, #shop.locations do
				...
				if shop.model then
					local ncoords = {x = coords.x, y = coords.y, z = coords.z - 1.0, heading = shop.heading}
					SpawnPed(shop.model, ncoords)
				end
