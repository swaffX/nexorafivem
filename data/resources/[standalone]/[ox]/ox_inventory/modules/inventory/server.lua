...
local function dropItem(source, playerInventory, fromData, data)
	...
	TriggerEvent("ria-logs:server:CreateLog", "yereatma", "", "orange", "``" .. GetPlayerName(Player.PlayerData.source) .. "(".. Player.PlayerData.source ..")``, adlı kişi  (**" .. data.count .. "x " .. toData.name .. "**) eşyayı yere attı.")					
				...
				if fromInventory.type == 'container' or sameInventory or totalWeight <= toInventory.maxWeight then
					...
					if not sameInventory then
						...
						if server.loglevel > 0 then
							...
							--TriggerEvent("ria-logs:server:CreateLog", "stashlog", "", "orange", "``" .. GetPlayerName(src) .. "(".. src ..")``, adlı kişi  (**" .. data.count .. "x " .. fromData.name .. "**) eşyayı  **"..fromInventory.label.."** depodan aldı.")
			...
			elseif data.count <= fromData.count then
				...
				if fromInventory.type == 'container' or sameInventory or (toInventory.weight + toData.weight <= toInventory.maxWeight) then
					...
					if not sameInventory then
						...
						if server.loglevel > 0 then
							--lib.logger(playerInventory.owner, 'swapSlots', ('%sx %s transferred from "%s" to "%s"'):format(data.count, fromData.name, fromInventory.owner and fromInventory.label or fromInventory.id, toInventory.owner and toInventory.label or toInventory.id))
							-- TriggerEvent("ria-logs:server:CreateLog", "stashlog", "", "orange", "``" .. toInventory.label .. "(".. src ..")``, adlı envanterden  (**" .. data.count .. "x " .. fromData.name .. "**) eşyayı  **".. fromInventory.label .."** depodan aldı.")
							...
							if string.match(fromInventory.id, "drop-") then
								...
								TriggerEvent("ria-logs:server:CreateLog", "yereatma", "", "orange", "``" .. GetPlayerName(Player.PlayerData.source) .. "(".. Player.PlayerData.source ..")`` adlı kişi  (**" .. data.count .. "x " .. toData.name .. "**) eşyayı yerden aldı.")								
							end

							if toInventory.type == "stash" and toInventory.type ~= "drop" and toInventory.type ~= "glovebox" and toInventory.type ~= "player" and fromInventory.type ~= "glovebox" and fromInventory.type ~= "drop"  then 
								TriggerEvent("ria-logs:server:CreateLog", "stashlog", "", "orange", "``" .. GetPlayerName(src) .. "(" .. src .. ")``" .. " adlı oyuncu " .. toInventory.label .. " deposuna **(" .. data.count .. "x " .. fromData.name .. ")** koydu!")
							else  
								if toInventory.type == "player" and toInventory.type ~= "stash" and toInventory.type ~= "drop" and toInventory.type ~= "glovebox" and fromInventory.type ~= "glovebox" and fromInventory.type ~= "drop"and fromInventory.type ~= "player"  then 
									TriggerEvent("ria-logs:server:CreateLog", "stashlog", "", "orange", "``" .. GetPlayerName(src) .. "(" .. src .. ")``" .. " adlı oyuncu " .. fromInventory.label .. " deposundan **(" .. data.count .. "x " .. toData.name .. ")** aldı!")
								end		
							end 

							-- if fromInventory.type == "drop" and toInventory.type == "player"  then 
							-- 	TriggerEvent("ria-logs:server:CreateLog", "yereatma", "", "orange", "``" .. GetPlayerName(src) .. "(" .. src .. ")``" .. " adlı oyuncu " .. data.count .. "x " .. toData.name .. " yerden aldı!")
							-- elseif fromInventory.type == "drop" and toInventory.type == "player"  then 
							-- 	TriggerEvent("ria-logs:server:CreateLog", "yereatma", "", "orange", "``" .. GetPlayerName(src) .. "(" .. src .. ")``" .. " adlı oyuncu " .. data.count .. "x " .. toData.name .. " yere attı!")
							-- end 

							if fromInventory.type == "glovebox" and toInventory.type == "player"  then 
								TriggerEvent("ria-logs:server:CreateLog", "torpidolog", "", "orange", "``" .. GetPlayerName(src) .. "(" .. src .. ")``" .. " adlı oyuncu " .. fromInventory.label .. " plakalı aracın torpidosundan **(" .. data.count .. "x " .. toData.name .. ")** aldı!")
							else
								if toInventory.type == "glovebox" and fromInventory.type == "player" then 
									TriggerEvent("ria-logs:server:CreateLog", "torpidolog", "", "orange", "``" .. GetPlayerName(src) .. "(" .. src .. ")``" .. " adlı oyuncu " .. toInventory.label .. " plakalı aracın torpidosuna **(" .. data.count .. "x " .. toData.name .. ")** koydu!")
								end 
							end 

							if toInventory.type == "player" and fromInventory.type == "player"  then 
								TriggerEvent("ria-logs:server:CreateLog", "ustarama", "", "orange", "``" .. GetPlayerName(src) .. "(" .. src .. ")``" .. " adlı oyuncu ``" ..  fromInventory.label .. "`` adlı oyuncunun üstünden **(" .. data.count .. "x " .. toData.name .. ")** aldı!")
							end
			...
			if Inventory.AddItem(toInventory, item, count, data.metadata, toSlot) then
				if Inventory.RemoveItem(fromInventory, item, count, data.metadata, slot) then
					if server.loglevel > 0 then
						...
						TriggerEvent('ria-logs:server:CreateLog', 'esyaverdirme', "" , "orange", "``" .. GetPlayerName(source) .. "(".. source ..")``, adlı kişi ``" .. GetPlayerName(target) .. "(".. target ..")``" .. " adlı kişiye (**" .. count .. "x " .. data.name .. "**) eşya verdi")
						-- print( fromInventory.label ..  count .. data.name .. toInventory.label )
					end

					-- local OtherPlayer = QBCore.Functions.GetPlayer(target)
					
					-- local giverName = GetPlayerName(src) .. "(".. src ..")"
					-- local receiverName = OtherPlayer.PlayerData.charinfo.firstname .. " " .. OtherPlayer.PlayerData.charinfo.lastname
		
					-- -- Log entry for item transfer
					-- local logMessage = string.format("``%s``, adlı kişi ``%s`` 'adlı kişiye (**%sx %s**) eşya verdi", giverName, receiverName, count, item.label)
					-- TriggerEvent("ria-logs:server:CreateLog", "esyaverdirme", "", "orange", logMessage)

					if item.name == "money" then
						local isParaAsimi = count >= 150000
						local ever = isParaAsimi and "@everyone" or ""
						local newMoney = tonumber(exports.qb-core:GetPlayer(target).PlayerData.money.cash) + tonumber(count)
						TriggerEvent('ria-logs:server:CreateLog', isParaAsimi and 'paraasimi' or "paragisicikisi", "" , "orange", "``" .. GetPlayerName(target) .. "(".. target ..") Kişiye gelen para: ".. count .."$ (cash) yeni parası: ".. newMoney .." `` ".. ever .."")
					end
					return
				end
			end
	...
	if type == 'table' and action == 'component' then
		...
		if weapon and weapon.metadata then
			...
			if action == 'load' and weapon.metadata.durability > 0 then
				...
				if not Inventory.RemoveItem(inventory, ammo, 1, specialAmmo) then return end
...
lib.callback.register('ox_inventory:removeAmmoFromWeapon', function(source, slot)
    ...
    -- Silah türüne göre mermi bölme faktörlerini tanımla
    local weaponDivideFactors = {
        weapon_pistol = 12,
        weapon_g17 = 12,
        weapon_g19 = 18,
        -- Buraya diğer silahları ekleyebilirsiniz
    }

    -- Silahın bölme faktörünü belirle (varsayılan 12)
    local divideFactor = weaponDivideFactors[item.name] or 12

    -- Envantere eklenecek mermi miktarını hesapla
    local ammoToAdd = math.floor(slotData.metadata.ammo / divideFactor)

    -- Eğer eklenecek mermi varsa, envantere ekle ve mermi miktarını güncelle
    if ammoToAdd > 0 and Inventory.AddItem(inventory, item.ammoname, ammoToAdd, { type = slotData.metadata.specialAmmo or nil }) then
        slotData.metadata.ammo = slotData.metadata.ammo % divideFactor
