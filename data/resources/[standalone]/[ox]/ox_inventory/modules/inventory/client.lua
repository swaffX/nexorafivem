...
function Inventory.OpenTrunk(entity)
    ...
    if GetVehicleDoorLockStatus(entity) > 1 then
        return false, lib.notify({ id = 'vehicle_locked', type = 'error', description = locale('vehicle_locked') })
    end
    ...
    if not client.openInventory('trunk', { id = invId, netid = NetworkGetNetworkIdFromEntity(entity), entityid = entity, door = door }) then return false end
