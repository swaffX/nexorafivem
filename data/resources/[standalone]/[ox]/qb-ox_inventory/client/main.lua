...
RegisterNetEvent(CurrentResourceName..':client:OpenStash', function(name)
    ...
    if not exports.ox_inventory:openInventory('stash', name) then
...
RegisterNetEvent(CurrentResourceName..':client:OpenShop', function(name)
    if not exports.ox_inventory:openInventory('shop', { type = name }) then
...
RegisterNetEvent(CurrentResourceName..':client:OpenOtherInventory', function(name)
    -- if GetInvokingResource() ~= CurrentResourceName then return end
    if not name then
        if not exports.ox_inventory:openNearbyInventory() then
            QBCore.Functions.Notify('You are not able to open nearby player\'s inventory now!', 'error')
        end
    else
        if not exports.ox_inventory:openInventory('player', name) then
