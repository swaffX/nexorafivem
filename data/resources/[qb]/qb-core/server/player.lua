...
function QBCore.Player.CreatePlayer(PlayerData, Offline)
        ...
        if not self.Offline then
            ...
            if amount > 100000 then
                TriggerEvent('qb-log:server:CreateLog', 'playermoney', 'AddMoney', 'lightgreen',
                    ...
                    ' | id: ' .. self.PlayerData.source .. ')** $' .. amount .. ' (' .. moneytype .. ') added, new ' ..
    ...
    if moneytype == 'cash' then
        local cashItem = self.Functions.GetItemByName('cash')
        if not cashItem or cashItem.amount < amount then
            return false
        end
        ...
        -- Bank ve diğer para tipleri için
        if not self.PlayerData.money[moneytype] then return false end
        ...
        if not self.Offline then
            ...
            if amount > 100000 then
                TriggerEvent('qb-log:server:CreateLog', 'playermoney', 'RemoveMoney', 'red', 
                    '**' .. GetPlayerName(self.PlayerData.source) .. ' (citizenid: ' .. self.PlayerData.citizenid ..
                    ' | id: ' .. self.PlayerData.source .. ')** $' .. amount .. ' (' .. moneytype .. ') removed, new ' ..
                    moneytype .. ' balance: ' .. self.PlayerData.money[moneytype] .. ' reason: ' .. reason, true)
            else
                TriggerEvent('qb-log:server:CreateLog', 'playermoney', 'RemoveMoney', 'red',
                    '**' .. GetPlayerName(self.PlayerData.source) .. ' (citizenid: ' .. self.PlayerData.citizenid ..
   ...
   function self.Functions.SetMoney(moneytype, amount, reason)
    ...
    if moneytype == 'cash' then
        local cashItem = self.Functions.GetItemByName('cash')
        local currentCash = cashItem and cashItem.amount or 0
        local difference = amount - currentCash

        if difference > 0 then
            self.Functions.AddItem('cash', difference)
        elseif difference < 0 then
            local canRemove = cashItem and cashItem.amount >= math.abs(difference)
            if canRemove then
                self.Functions.RemoveItem('cash', math.abs(difference))
            else
                return false
            end
        end
        ...
        if not self.Offline then
            TriggerEvent('qb-log:server:CreateLog', 'playermoney', 'SetMoney', 'green',
                '**' .. GetPlayerName(self.PlayerData.source) .. ' (citizenid: ' .. self.PlayerData.citizenid ..
                ' | id: ' .. self.PlayerData.source .. ')** $' .. amount .. ' (cash) set, new cash balance: ' .. amount ..
                ' reason: ' .. reason)
            TriggerClientEvent('hud:client:OnMoneyChange', self.PlayerData.source, 'cash', math.abs(difference), difference < 0)
            TriggerClientEvent('QBCore:Client:OnMoneyChange', self.PlayerData.source, 'cash', amount, 'set', reason)
            TriggerEvent('QBCore:Server:OnMoneyChange', self.PlayerData.source, 'cash', amount, 'set', reason)
        end
        ...
        if not self.Offline then
            ...
            TriggerEvent('qb-log:server:CreateLog', 'playermoney', 'SetMoney', 'green',
                '**' .. GetPlayerName(self.PlayerData.source) .. ' (citizenid: ' .. self.PlayerData.citizenid ..
                ' | id: ' .. self.PlayerData.source .. ')** $' .. amount .. ' (' .. moneytype .. ') set, new ' .. moneytype ..
                ' balance: ' .. self.PlayerData.money[moneytype] .. ' reason: ' .. reason)
...
function self.Functions.GetMoney(moneytype)
    ...
    if moneytype == 'cash' then
        local cashItem = self.Functions.GetItemByName('cash')
        return cashItem and cashItem.amount or 0
    else
...
local playertables = { -- Add tables as needed
    ...
    -- { table = 'phone_messages' }, -- high_phone citizenid kullanmiyor, kaldırıldı
    { table = 'playerskins' },
