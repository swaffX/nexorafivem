...
local function ParkOwnedVehicle(veh, garageName, vehLocation, plate)
    ...
    if FuelScript and GetResourceState(FuelScript):find('start') then
    ...
    elseif GetResourceState('LegacyFuel'):find('start') then
        local ok, fuel = pcall(function()
...
local function UpdateVehicleSpawnerSpawnedVehicle(veh, garage, heading, cb)
    ...
    if FuelScript then
        ...
        exports['cc-fuel']:SetFuel(veh, 100) -- Don't change this. Change it in the  Defaults to legacy fuel if not set in the config
    end
...
function UpdateSpawnedVehicle(spawnedVehicle, vehicleInfo, heading, garage, properties)
    ...
    if garage.useVehicleSpawner then
        ...
        if FuelScript then
            ...
            exports['cc-fuel']:SetFuel(spawnedVehicle, 100) -- Don't change this. Change it in the  Defaults to legacy fuel if not set in the config
        end
        TriggerEvent("vehiclekeys:client:SetOwner", plate)
        TriggerServerEvent("qb-garage:server:UpdateSpawnedVehicle", plate, true)
    else
        ...
        if FuelScript then
            ...
            exports['cc-fuel']:SetFuel(spawnedVehicle, vehicleInfo.fuel) -- Don't change this. Change it in the  Defaults to legacy fuel if not set in the config
        end
...
RegisterNetEvent('qb-garages:client:OpenMenu', function()
    ...
    if CurrentGarage then
        ...
        local categories = garage.vehicleCategories or { 'car' }
        local superCategory = GetSuperCategoryFromCategories(categories)
        if type == 'job' and garage.useVehicleSpawner then
            ...
            OpenGarageVehicleMenu(
                ...
                type,
                categories,
                Lang:t("menu.header." .. garage.type .. "_" .. superCategory, { value = garage.label }),
                superCategory
            )
        end
...
RegisterNetEvent('qb-garages:client:OpenHouseGarage', function()
    ...
    if UseLoafHousing then
        ...
        if hasKey then
            OpenGarageVehicleMenu(CurrentHouseGarage, HouseGarages[CurrentHouseGarage], 'house', HouseGarageCategories, HouseGarages[CurrentHouseGarage].label, superCategory)
        else
