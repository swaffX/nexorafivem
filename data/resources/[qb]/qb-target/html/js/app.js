            ...
            setTimeout(() => {
                for (let [index, itemData] of Object.entries(item.data)) {
                    if (itemData !== null) {
                        ...
                        $('#f9tg_'+index).html(`
  <div class="f9-theicon">
    <i id="target-option_${index}" class="${itemData.icon}" data-label="${itemData.label}"></i>
  </div>
  <div id="target-text_${index}" class="f9-titleofoption"></div>
`);

setTimeout(() => {
  $('[id^="f9tg_"]').hover(
    function () {
      const label = $(this).find('i').data('label');
      $('#global-target-label').text(label).fadeIn(100);
    },
    function () {
      $('#global-target-label').fadeOut(100);
    }
  );
}, 150); // DOM render sonrası
        ...
        LeftTarget() {
            ...
            $('#target-eye').css('transform','translateY(-50%) translateX(-50%) scale(0.9)').css('opacity','1');
             $('#global-target-label').hide(); // label'ı gizle
            closecursor();
            // $(".linenumber2").animate({ height: '0vh' }, 50);
            setTimeout(() => {
