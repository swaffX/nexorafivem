...
RegisterNetEvent('qb-inventory:client:updateInventory', function()
    ...
    SendNUIMessage({
        ...
        inventory = items,
        playerinfo = {
            name = PlayerData.charinfo.firstname .. " " .. PlayerData.charinfo.lastname, -- Assuming name is in charinfo.firstname and lastname
            id = PlayerData, -- Assuming ID is citizenid
            cash = PlayerData.money.cash -- Assuming cash is in money.cash
        }
    })
end)
...
RegisterNetEvent('qb-inventory:client:openInventory', function(items, other, TargetId)
    ...
    SendNUIMessage({
        ...
        playerinfo = {
            name = PlayerData.charinfo.firstname .. " " .. PlayerData.charinfo.lastname, -- Assuming name is in charinfo.firstname and lastname
            id = GetPlayerServerId(PlayerId()), -- Assuming ID is citizenid
            cash = PlayerData.money.cash, -- Assuming cash is in money.cash
...
RegisterNUICallback("setSleeve", function(data, cb)
    local sleeveValue = tonumber(data.sleeve)
    if sleeveValue then
        -- Illenium appearance için kol slotlarını değiştir
        local ped = PlayerPedId()

        -- örnek: kol kısmı genelde 3. komponent (arms)
        SetPedComponentVariation(ped, 3, sleeveValue, 0, 2)

        print(("Kol ayarlandı: %s"):format(sleeveValue))
    end
    cb("ok")
end)
...
RegisterNUICallback('GetCurrentOutfit', function(data, cb)
    ...
    local outfit = PlayerData.metadata.outfit or {} -- Oyuncunun giyili kıyafet bilgilerini metadata'dan al
    cb(outfit)
end)

RegisterNUICallback('SetClothingVariation', function(data, cb)
    local ped = PlayerPedId()
    exports['illenium-appearance']:SetComponentVariation(ped, data.componentId, data.drawableId, data.textureId, data.paletteId)
    cb(true)
end)

-- YENİ: Kıyafet itemlarının detaylarını NUI'ye döndürmek için
-- NUI'den sadece component bilgileriyle item detaylarını isteyeceğiz
RegisterNUICallback('GetClothingItemDetails', function(data, cb)
    local outfitDetails = {}
    if data.outfitData then
        for componentId, clothingInfo in pairs(data.outfitData) do
            -- shared/items.lua'dan tüm itemları döngüye sokup
            -- eşleşen kıyafet itemını bulmamız gerekiyor.
            -- Bu işlem CPU yoğun olabilir, optimize edilebilir.
            for itemName, itemData in pairs(QBCore.Shared.Items) do
                if itemData.info and itemData.info.clothingType and
                   itemData.info.component == tonumber(componentId) and
                   itemData.info.drawable == clothingInfo.drawable and
                   itemData.info.texture == clothingInfo.texture then
                    outfitDetails[componentId] = itemData -- Tüm item detayını döndür
                    break -- Eşleşeni bulduk, döngüden çık
                end
            end
        end
    end
...
-- NUI'ye item ekleme/çıkarma callback'lerini kontrol edin, bunlar zaten olmalı
RegisterNUICallback('AddItem', function(data, cb)
    local Player = QBCore.Functions.GetPlayer(source)
    if Player then
        -- info parametresini de ekleyerek AddItem fonksiyonuna iletin
        local success = Player.Functions.AddItem(data.item.name, data.amount, data.info, data.slot) -- data.info eklendi
        cb(success)
    else
        cb(false)
    end
...
RegisterNUICallback('RemoveItem', function(data, cb)
    local Player = QBCore.Functions.GetPlayer(source)
    if Player then
        local success = Player.Functions.RemoveItem(data.item.name, data.amount, data.slot)
        cb(success)
    else
        cb(false)
    end
