...
local function HasItem(items, amount)
    ...
    local count = 0
    local kvIndex = 2
    local playerItems = (PlayerData and PlayerData.items) or {}
...
RegisterNetEvent('qb-inventory:client:giveAnim2', function()
    Wait(2000)
    LoadAnimDict('mp_common')
	TaskPlayAnim(PlayerPedId(), 'mp_common', 'givetake1_b', 8.0, 1.0, -1, 16, 0, 0, 0, 0)
    ExecuteCommand('me Uzatılan eşyayı alır.')
end)
...
QBCore.EnumerateEntities = function(initFunc, moveFunc, disposeFunc)
...
QBCore.GetVehicles = function()
    local vehicles = {}
    for vehicle in BQ.EnumerateEntities(FindFirstVehicle, FindNextVehicle, EndFindVehicle) do
...
QBCore.GetPlayersInArea = function(coords, area)
    local players, playersInArea = QBCore.GetPlayers(), {}
...
QBCore.GetPlayersFromCoords = function(coords, distance)
...
QBCore.GetClosestPlayer = function()
    local closestPlayers = QBCore.GetPlayersFromCoords()
...
RegisterNUICallback('GetNearPlayers', function(data, cb)
    ...
    local players, nearbyPlayer = QBCore.GetPlayersInArea(GetEntityCoords(playerPed), 2.0)
    local player, distance = QBCore.GetClosestPlayer()
    ...
    for i = 1, #players, 1 do
        if players[i] ~= PlayerId() then
            table.insert(players_clean, { name = '('..GetPlayerServerId(players[i])..') '..GetPlayerName(players[i]), id = GetPlayerServerId(players[i]) })
