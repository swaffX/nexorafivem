...
--[[ 				secondInv.label = ""..other.label..'<span style="color:#64e778;''- Nakit: '..para.."$</span>" ]]
			...
			elseif name == "otherplayer" then
				...
				if Drops[id] and not Drops[id].isOpen then
					...
					secondInv.name = id
					secondInv.label = '<i class="fas fa-box-open"></i>'.." Yer-"..tostring(id)
...
RegisterNetEvent('inventory:server:SetInventoryData', function(fromInventory, toInventory, fromSlot, toSlot, fromAmount, toAmount)
		...
		if fromItemData and fromItemData.amount >= fromAmount then
			...
			elseif QBCore.Shared.SplitStr(toInventory, "-")[1] == "traphouse" then
				...
				if IsItemValid then
					...
					QBCore.Functions.Notify(src, "Bu öğeyi satamazsınız.", 'error')
				end
			else
				-- drop
			...
			QBCore.Functions.Notify(src, "Bu öğeye sahip değilsiniz!", "error")
		end
	elseif QBCore.Shared.SplitStr(fromInventory, "-")[1] == "otherplayer" then
		...
		if fromItemData and fromItemData.amount >= fromAmount then
			...
			QBCore.Functions.Notify(src, "İtem Bulunamadı", "error")
		end
	elseif QBCore.Shared.SplitStr(fromInventory, "-")[1] == "trunk" then
		...
		if fromItemData and fromItemData.amount >= fromAmount then
			...
			QBCore.Functions.Notify(src, "Öğe mevcut değil!", "error")
		end
		...
		if QBCore.Shared.SplitStr(shopType, "_")[1] == "Dealer" then
			if QBCore.Shared.SplitStr(itemData.name, "_")[1] == "weapon" then
				...
				if Player.Functions.RemoveMoney("cash", price, "dealer-item-bought") then
					...
					QBCore.Functions.Notify(src, itemInfo["label"] .. " Satın Alındı!", "success")
					...
					QBCore.Functions.Notify(src, "Yeterli paranız yok.", "error")
				end
			else
				if Player.Functions.RemoveMoney("cash", price, "dealer-item-bought") then
	...
	elseif fromInventory == "crafting" then
		...
		if hasCraftItems(src, itemData.costs, fromAmount) then
			...
			QBCore.Functions.Notify(src, "Doğru öğelere sahip değilsiniz.", "error")
		end
...
QBCore.Commands.Add("soy", "Oyuncuyu Soy", {}, false, function(source, args)
	...
	if amount <= item.amount then
		...
		if RemoveItem(src, item.name, amount, item.slot) then
			if AddItem(target, item.name, amount, false, item.info) then
				...
				QBCore.Functions.Notify(target, "QBCore:Notify", Player.PlayerData.charinfo.firstname.." "..Player.PlayerData.charinfo.lastname.." kişisinden "..amount.." adet "..item.label.." aldın.")
				TriggerClientEvent("inventory:client:UpdatePlayerInventory", target, true)
				TriggerClientEvent('inventory:client:ItemBox',src, QBCore.Shared.Items[item.name], "remove")
				QBCore.Functions.Notify(src, OtherPlayer.PlayerData.charinfo.firstname.." "..OtherPlayer.PlayerData.charinfo.lastname.." kişisine "..amount.." adet "..item.label.." verdin.")
				TriggerClientEvent("inventory:client:UpdatePlayerInventory", src, true)
				TriggerClientEvent('qb-inventory:client:giveAnim', src)
				TriggerClientEvent('qb-inventory:client:giveAnim2', target)
			else
			...
			TriggerClientEvent('QBCore:Notify', src,  "Öğeden yeterince sahip değilsin", "error")
		end
	else
