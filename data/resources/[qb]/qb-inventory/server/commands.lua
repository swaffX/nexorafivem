...
QBCore.Commands.Add('testoutfit', 'Oyuncuya test kıyafetleri verir.', {}, false, function(source, args)
    local Player = QBCore.Functions.GetPlayer(source)

    if Player then
        -- Erkek Tişörtü (Lütfen kendi component/drawable/texture değerlerinizle güncelleyin!)
        Player.Functions.AddItem('erkek_tisortu_siyah', 1, {
            clothingType = true,
            component = 3,  -- Tişört için component ID
            drawable = 15,  -- Tişörtün drawable ID'si (Örnek: Lütfen kendi değerinizle değiştirin)
            texture = 0,    -- Tişörtün texture ID'si (Örnek: Lütfen kendi değerinizle değiştirin)
            palette = 0
        })
        TriggerClientEvent('QBCore:Notify', source, 'Siyah Tişört envanterine eklendi!', 'success')

        -- Mavi Kot Pantolon (Lütfen kendi component/drawable/texture değerlerinizle güncelleyin!)
        Player.Functions.AddItem('kadin_pantolonu_kot', 1, {
            clothingType = true,
            component = 4,  -- Pantolon için component ID
            drawable = 20,  -- Pantolonun drawable ID'si (Örnek: Lütfen kendi değerinizle değiştirin)
            texture = 1,    -- Pantolonun texture ID'si (Örnek: Lütfen kendi değerinizle değiştirin)
            palette = 0
        })
        TriggerClientEvent('QBCore:Notify', source, 'Mavi Kot Pantolon envanterine eklendi!', 'success')
    else
