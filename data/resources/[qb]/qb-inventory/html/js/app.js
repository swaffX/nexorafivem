    ...
    $.post(
        ...
        function (data) {
            ...
            $(".weapon-attachments-container-details").html(
                ...
                '<br><br><span style="font-weight: bold; letter-spacing: .1vh;">Sağlamlık - ' +
                Durability.toFixed() +
            ...
            if (data.AttachmentData !== null && data.AttachmentData !== undefined) {
                if (data.AttachmentData.length > 0) {
                    ...
                    $(".weapon-attachments-title").html(
                        '<span style="font-weight: bold; letter-spacing: .1vh;">Bu silahta eklenti içermiyor.</span>'
                    );
                }
            } else {
                $(".weapon-attachments-title").html(
...
$(document).on("click", "#weapon-attachments", function (e) {
    ...
    if (!Inventory.IsWeaponBlocked(ClickedItemData.name)) {
        ...
        $.post(
            ...
            JSON.stringify({
                message: "Addons are not available for this weapon.",
                type: "error",
            })
        );
    }
    ...
    if (itemData != null && itemData.info != "") {
        if (itemData.name == "id_card") {
            ...
            $(".item-info-description").html(
                "<p><strong>Vatandaş ID: </strong><span>" +
                ...
                gender +
                "</span></p><p><strong>Ülke: </strong><span>" +
            ...
            $(".item-info-description").html(
                ...
                "</span></p><p><strong>Doğum Tarihi: </strong><span>" +
                itemData.info.birthdate +
                "</span></p><p><strong>Lisans Türü: </strong><span>" +
                itemData.info.type +
                "</span></p>"
            );
            ...
            $(".item-info-description").html(
                ...
                "</span></p><p><strong>Soyisim: </strong><span>" +
                itemData.info.lastname +
                "</span></p><p><strong>Vatandaş ID: </strong><span>" +
                itemData.info.citizenid +
                "</span></p>"
            );
            ...
            if (itemData.info.type == "casing") {
                $(".item-info-description").html(
                    ...
                    "</span></p><p><strong>Seri No: </strong><span>" +
                    itemData.info.serie +
        ...
        } else if (itemData.name == "heardring") {
            $(".item-info-title").html('<p>' + itemData.label + '</p>')
            $(".item-info-description").html('<p><strong>Eşi: </strong><span>' + itemData.info.aile + '</span></p><p><strong>Tarih: </strong><span>' + itemData.info.tarih + '</span></p>');
        } else {
            $(".item-info-title").html("<p>" + itemData.label + "</p>");
    ...
    if (fromData !== undefined && fromData.amount >= $toAmount) {
        ...
        if (
            ...
            if (fromData.amount == $toAmount) {
                ...
                } else if ($toSlot == 46 && $toInv.attr("data-inventory") == "player") {
                    $toInv
                        .find("[data-slot=" + $toSlot + "]")
                        .html(
...
(() => {
    ...
    Inventory.UseItem = function (data) {
        ...
        $("#itembox-action").html("<p>Use 1x</p>");
    ...
    Inventory.itemBox = function (data) {
        ...
        if (type == "Kullanıldı") {
            $itembox.css({ "background-color": "rgba(34, 35, 51, 0.80)" });
        } else if (type == "Eklendi") {
            $itembox.css({ "background-color": "rgb(12 64 6 / 80%)" });
        } else if (type == "Silindi") {
            $itembox.css({ "background-color": "rgb(56 0 0 / 80%)" });
        }
        ...
        if (data.toggle) {
            if (!requiredItemOpen) {
                ...
                $.each(data.items, function (index, item) {
                    var element = '<div class="requiredItem-box"><div id="requiredItem-action">Gerekli</div><div id="requiredItem-label"><p>' + item.label + '</p></div><div id="requiredItem-image"><div class="item-slot-img"><img src="images/' + item.image + '" alt="' + item.name + '" /></div></div></div>'
                    $(".requiredItem-container").hide();
    ...
    window.onload = function (e) {
        window.addEventListener('message', function (event) {
            ...
            if (event.data.action == "nearPlayersGive") {
                ////successAudio.play();
                givingItem = event.data.ItemId;
                $('.near-players-wrapper').find('.popup-body').html('');
                $.each(event.data.players, function (index, player) {
                    $('.near-players-list .popup-body').append(`<div class="player" data-id="${player.id}" data-action="${event.data.action}">${player.name}</div>`);
                });
