      ...
      sharemenu.OnItemSelect = function(sender, item, index)
        if ShareTable[index] ~= 'none' then
          ...
          if(distance ~= -1 and distance < 3) then
            ...
            QBCore.Functions.Notify(Config.Languages[lang]['sentrequestto']..GetPlayerName(target))
          else
            QBCore.Functions.Notify(Config.Languages[lang]['nobodyclose'])
          end
        end
      end
...
function AddWalkMenu(menu)
    ...
    WalkInjured = NativeUI.CreateItem("Yaralı", "")
