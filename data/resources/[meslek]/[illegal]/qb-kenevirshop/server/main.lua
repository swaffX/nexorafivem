...
RegisterNetEvent("qb-kenevirshop:server:sellPawnItemss", function(itemName, itemAmount, itemPrice)
    local src = source
    ...
    if Player.Functions.RemoveItem(itemName, tonumber(itemAmount)) then
        if Config.BankMoney then
			...
			Player.Functions.AddItem('markedbills', itemAmount, false, info)
        end
...
RegisterNetEvent("qb-kenevirshop:server:meltItemRemove", function(itemName, itemAmount,item)
    local src = source
    local Player = QBCore.Functions.GetPlayer(src)
    local meltTime = 0
    ...
    if Player.Functions.RemoveItem(itemName, itemAmount) then
        ...
        TriggerClientEvent('qb-kenevirshop:client:startMelting', src, item, tonumber(itemAmount), (meltTime* 60000/1000))
...
RegisterNetEvent("qb-kenevirshop:server:pickupMelted", function(item)
    local src = source
    local Player = QBCore.Functions.GetPlayer(src)
    ...
    TriggerClientEvent('qb-kenevirshop:client:resetPickup', src)
end)

QBCore.Functions.CreateCallback('qb-kenevirshop:server:getInv', function(source, cb)
