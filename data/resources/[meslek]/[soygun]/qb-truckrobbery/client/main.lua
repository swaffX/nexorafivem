    ...
    while true do
        ...
        if dist <= 1.0 then
            QBCore.Functions.DrawText3D(MissionMarker.x, MissionMarker.y, MissionMarker.z,
                "~g~[E]~b~ Görevleri Kabut Etmek İçin")
...
RegisterNetEvent('AttackTransport:InfoForLspd', function(x, y, z)
    if PlayerJob ~= nil and PlayerJob.name == 'police' then
        ...
        if PoliceBlip == 0 then
            ...
            AddTextComponentString('Nakit taşımaya saldırı')
        ...
        if PoliceDist <= 4.5 then
            ...
            if SilenceAlarm == 0 then
                hintToDisplay('Alarmı susturmak için ~INPUT_DETONATE~ tuşuna basın')
                SilenceAlarm = 1
            end
...
RegisterNetEvent('qb-armoredtruckheist:client:robberyCall', function(streetLabel, coords)
    if PlayerJob.name == "police" then
        local store = "Zırhlı kamyon"

        PlaySound(-1, "Lose_1st", "GTAO_FM_Events_Soundset", 0, 0, 1)
        TriggerEvent('qb-policealerts:client:AddPoliceAlert', {
            timeOut = 10000,
            alertTitle = "Zırhlı Araç Soygun Girişimi",
            coords = {
                x = coords.x,
                y = coords.y,
                z = coords.z
        ...
        BeginTextCommandSetBlipName('STRING')
        AddTextComponentString("10-90: Zırhlı Kamyon Soygunu")
        EndTextCommandSetBlipName(blip)
        while transG ~= 0 do
            Wait(180 * 4)
            transG = transG - 1
...
function MissionNotification()
    ...
    TriggerServerEvent('qb-phone:server:sendNewMail', {
        sender = "Patron",
        subject = "Yeni Hedef",
        message = "Yani biraz para kazanmakla ilgileniyor musun? iyi... git kendine bir silah al ve bunu gerçekleştir... sana şimdi konumu gönderiyorum."
    })
    Citizen.Wait(3000)
end
---
    ...
    while true do
        ...
        if MissionStart == 1 then
            ...
            if dist <= 55.0 then
                ...
                if warning == 0 then
                    warning = 1
                    QBCore.Functions.Notify("Bombayı yerleştirmeden önce gardiyanlardan kurtulun.", "error")
                end
            ...
            if dist <= 7 and BlownUp == 0 and PlayerJob.name ~= 'police' then
                if BlowBackdoor == 0 then
                    hintToDisplay('Arka kapıyı havaya uçurmak ve parayı almak için [G] tuşuna basın')
                    BlowBackdoor = 1
                end
                if IsControlPressed(0, 47) and GuardsDead == 1 then
    ...
    if IsVehicleStopped(transport) then
        if IsVehicleSeatFree(transport, -1) and IsVehicleSeatFree(transport, 0) and IsVehicleSeatFree(transport, 1) and
            ...
            if not IsEntityInWater(PlayerPedId()) then
                ...
                AttatityToEntity(prop, PlayerPedId(), GetPedBoneIndex(PlayerPedId(), 60309), 0.06, 0.0, 0.06, 90.0,
                ...
                Detatity(prop)
                AttatityToEntity(prop, transport, GetEntityBoneIndexByName(transport, 'door_pside_r'), -0.7, 0.0,
                ...
                BlownUp = 1
                lootable = 1
                QBCore.Functions.Notify('Nakit toplamaya başlayabilirsiniz.', "success")
                RemoveBlip(TruckBlip)
            else
                QBCore.Functions.Notify('Sudan çık', "error")
            end
        else
            QBCore.Functions.Notify('Yükü yerleştirmek için araç boş olmalıdır', "error")
        end
    else
        QBCore.Functions.Notify('Hareket halindeki bir aracı soyamazsınız', "error")
    end
end
        ...
        if lootable == 1 then
            ...
            if dist <= 4.5 then
                if PickupMoney == 0 then
                    hintToDisplay('Parayı almak için [E] tuşuna basın')
                    PickupMoney = 1
                end
                if IsControlJustPressed(0, 38) then
                    lootable = 0
...
function TakingMoney()
    ...
    AttatityToEntity(bag, PlayerPedId(), GetPedBoneIndex(PlayerPedId(), 57005), 0.0, 0.0, -0.16, 250.0, -30.0, 0.0,
        false, false, false, false, 2, true)
    TaskPlayAnim(PlayerPedId(), "anim@heists@ornate_bank@grab_cash_heels", "grab", 8.0, -8.0, -1, 1, 0, false, false,
        false)
    FreezeEntityPosition(PlayerPedId(), true)
    QBCore.Functions.Notify('Bir çantaya nakit para koyuyorsun', "success")
    local _time = GetGameTimer()
    while GetGameTimer() - _time < 20000 do
        if IsControlPressed(0, 47) then
            break
        end
        hintToDisplay('Kurtarmak için [G] düğmesini basılı tutun')
        Citizen.Wait(1)
    end
