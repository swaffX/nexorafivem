...
local drift_speed_limit = 7100  -- yanlama hızı
local toggle = 79 -- Numpad 9
...
Citizen.CreateThread(function()
	...
	while true do
		...
		if IsControlJustPressed(79, 79) then

			driftmode = not driftmode

			if driftmode then
				QBCore.Functions.Notify('Drift Modu Aktif', 'success')
			else
				QBCore.Functions.Notify('Drift Modu Kapalı', 'error')
			end
		end
