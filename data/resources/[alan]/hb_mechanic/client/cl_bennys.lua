...
local customsCam = nil -- Global camera variable for custom camera management
...
local function saveVehicle()
    ...
    TriggerServerEvent('hb_mechanic:server:updateVehicle', myCar)
end
...
function AttemptPurchase(type, upgradeLevel)
    ...
    TriggerServerEvent("hb_mechanic:server:attemptPurchase", type, upgradeLevel)
...
function ExitBennys()
    ...
    -- Disable mouse cursor when exiting
    SetNuiFocus(false, false)
    
    -- Destroy camera system
    RenderScriptCams(false, false, 0, true, false)
    if customsCam ~= nil then
...
function EnterLocation(override)
    ...
    SetTimeout(100, function()
        ...
        -- Initialize camera system (global)
        if customsCam ~= nil then
            DestroyCam(customsCam, false)
            customsCam = nil
        end
        ...
        local vehHeading = GetEntityHeading(plyVeh)
        local camX = vehCoords.x - (math.sin(math.rad(vehHeading)) * 3.0)
        local camY = vehCoords.y - (math.cos(math.rad(vehHeading)) * 3.0)
        local camZ = vehCoords.z + 1.0
        
        SetCamCoord(customsCam, camX, camY, camZ)
        PointCamAtCoord(customsCam, vehCoords.x, vehCoords.y, vehCoords.z)
...
function DisableControls(repairOnly)
    ...
    local cameraDistance = 3.0
    local minDistance = 1.0
    local maxDistance = 10.0
    local cameraAngle = 0.0 -- Camera rotation angle
    local fixedCameraCoords = nil -- Store fixed camera position when exiting camera mode
    
    -- Add event listener for camera mode toggle
    AddEventHandler('hb_mechanic:client:toggleCameraMode', function()
        cameraMode = not cameraMode
        print("DEBUG: Camera mode toggle event tetiklendi, cameraMode:", cameraMode)
        if cameraMode then
            SetNuiFocus(false, false) -- Disable mouse cursor
            print("DEBUG: Mouse kapandı, kamera moduna geçildi")
            PlaySoundFrontend(-1, "NAV_UP_DOWN", "HUD_FRONTEND_DEFAULT_SOUNDSET", 1)
            -- Clear fixed camera position when entering camera mode
            fixedCameraCoords = nil
        else
            SetNuiFocus(true, true) -- Enable mouse cursor
            print("DEBUG: Mouse açıldı, menü moduna geçildi")
            PlaySoundFrontend(-1, "NAV_UP_DOWN", "HUD_FRONTEND_DEFAULT_SOUNDSET", 1)
            -- Store current camera position as fixed when exiting camera mode
            if customsCam ~= nil then
                fixedCameraCoords = GetCamCoord(customsCam)
                print("DEBUG: Kamera sabitlendi, pozisyon:", fixedCameraCoords)
            end
        end
    end)
    ...
    CreateThread(function()
        while isPlyInBennys do
            ...
            -- Removed arrow key controls (172, 173) - now using mouse control
            ...
            -- Camera controls in camera mode
            if cameraMode and customsCam ~= nil then
                -- Check for right click to exit camera mode
                if IsControlJustReleased(1, 25) then -- Right click
                    cameraMode = false
                    SetNuiFocus(true, true) -- Enable mouse cursor
                    print("DEBUG: Kamera modunda sağ tık algılandı, mouse moduna geçildi")
                    PlaySoundFrontend(-1, "NAV_UP_DOWN", "HUD_FRONTEND_DEFAULT_SOUNDSET", 1)
                    -- Store current camera position as fixed when exiting camera mode
                    fixedCameraCoords = GetCamCoord(customsCam)
                    print("DEBUG: Kamera sabitlendi, pozisyon:", fixedCameraCoords)
                ...
                if scroll > 0 then
                    cameraDistance = math.max(minDistance, cameraDistance - 0.5)
                elseif scroll < 0 then
                    cameraDistance = math.min(maxDistance, cameraDistance + 0.5)
                end
                
                -- Camera rotation with mouse movement
                local mouseX = GetDisabledControlNormal(1, 1) -- Mouse X movement
                if math.abs(mouseX) > 0.1 then
                    cameraAngle = cameraAngle + (mouseX * 5.0) -- Increased sensitivity from 2.0 to 5.0
                end
                ...
                local totalAngle = math.rad(vehHeading + cameraAngle)
                local camX = vehCoords.x - (math.sin(totalAngle) * cameraDistance)
                local camY = vehCoords.y - (math.cos(totalAngle) * cameraDistance)
                local camZ = vehCoords.z + 1.0
                
                SetCamCoord(customsCam, camX, camY, camZ)
                PointCamAtCoord(customsCam, vehCoords.x, vehCoords.y, vehCoords.z)
            elseif not cameraMode and customsCam ~= nil and fixedCameraCoords ~= nil then
                -- Keep camera fixed at last position when not in camera mode
                SetCamCoord(customsCam, fixedCameraCoords.x, fixedCameraCoords.y, fixedCameraCoords.z)
                -- Keep pointing at vehicle
            ...
            -- Removed Backspace key handling - right click now only toggles camera mode
...
function GetLocations()
    QBCore.Functions.TriggerCallback("hb_mechanic:server:GetLocations", function(locations)
    ...
    if Config.UseRadial then
        if not radialMenuItemId then
            radialMenuItemId = exports['qb-radialmenu']:AddOption({
                ...
                icon = 'wrench',
                type = 'client',
                event = 'hb_mechanic:client:EnterCustoms',
                shouldClose = true
            })
        end
    else
...
RegisterNetEvent('hb_mechanic:client:UpdateLocation', function(location, type, key, value)
    Config.Locations[location][type][key] = value
end)

RegisterNetEvent("hb_mechanic:client:purchaseSuccessful", function()
    isPurchaseSuccessful = true
    attemptingPurchase = false
    QBCore.Functions.Notify("Purchase Successful")
end)

RegisterNetEvent("hb_mechanic:client:purchaseFailed", function()
    isPurchaseSuccessful = false
    attemptingPurchase = false
    QBCore.Functions.Notify("Not enough money", "error")
end)

RegisterNetEvent('hb_mechanic:client:EnterCustoms', function(override)
