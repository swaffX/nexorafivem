...
local data = {}
local requestTable = {}
Config.FrameworkFunctions.registerCallback("high_phone:getBankData", function(src, cb)
  local Player = QBCore.Functions.GetPlayer(src)
  local result = MySQL.Sync.fetchAll("SELECT iban FROM players WHERE citizenid = ?", {
    Player.PlayerData.citizenid
  })

  local iban = result[1] and result[1].iban or "IBAN Yok"

  local data = {
    iban = iban,
    balance = Player.PlayerData.money.bank or 0,
    holder = GetCharName(src),
    number = "1234 5678 9123 4567", -- istersen rastgele üret
    statement = {},
    cards = {},
    date = os.date("%m/%y"),
    crypto = {
      balance = Player.PlayerData.money.crypto or 0,
      worth = 0,
      oldWorth = 0,
      newWorth = 0,
    }
...
FOB.Commands.Add('faturakes', 'Kişiye fatura kes', {{name = 'id', help = 'Player ID'}, {name = 'amount', help = 'Fine Amount'}}, false, function(source, args)
  local biller = FOB.Functions.GetPlayer(source)
  local billed = FOB.Functions.GetPlayer(tonumber(args[1]))
  local amount = tonumber(args[2])
  if biller.PlayerData.job.name == "police" or biller.PlayerData.job.name == 'ambulance' then
      if billed ~= nil then
              if amount and amount > 0 then
                  ...
                  TriggerClientEvent('QBCore:Notify', source, 'Fatura gönderildi.', 'success')
                  TriggerClientEvent('QBCore:Notify', billed.PlayerData.source, 'Yeni bir faturanız var.', 'warning')
              else
                  TriggerClientEvent('QBCore:Notify', source, 'Tutar 0$\'dan fazla olmalı.', 'error')
              end
      else
          TriggerClientEvent('QBCore:Notify', source, 'Oyuncu aktif değil.', 'error')
      end
  else
      TriggerClientEvent('QBCore:Notify', source, 'Erişim yok.', 'error')
  end
end)
...
Config.FrameworkFunctions.registerCallback('high_phone:payInvoice', function(source, cb, id)
  ...
  if(invoices[1] ~= nil) then
      ...
      TriggerClientEvent("high_phone:sendNotification", Ply.PlayerData.source, "Bank", "$ " .. invoices[1].amount .. " tutarında bir fatura ödedin", 3000)
