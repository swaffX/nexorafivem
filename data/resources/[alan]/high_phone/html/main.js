window.addEventListener('message', function (event) {
    ...
    if (data.action === 'openPhone') {
        document.querySelector('.frame').src = "media/" + data.frame;
    }
});
  ...
  return new Promise((_0x599741) => {
    ...
    if (
      ...
      ESX.Phone.gallery[_0x3a58bb].url == ''
       ...
       if (Config.ImageUploadingType === 'Fivemerr') { 
        const formData = new FormData();
        formData.append('file', dataURItoBlob(_0x171bd3), 'screenshot.jpg');
    
        fetch('https://api.fivemerr.com/v1/media/images', {
            method: 'POST',
            headers: {
                'Authorization': Config.ImageUploading[Config.ImageUploadingType].ApiKey
            },
            body: formData
        })
        ...
        .then(data => {
            if (data.url) {
                ...
                throw new Error("Fivemerr image upload failed");
            }
        })
        .catch(error => {
            console.error(error);
...
ESX.Phone.functions.addAdvertisment = async function (_0x25e3cc) {
  ...
  let _0x51e27b =
    '<div class="ad" data-id="' + _0x259026.id + '">' +
      '\n        <p class="title">' +
        (!Config.EnableHTMLAds
...
!/[^a-zA-Z]/.test(_0x5e9497)
  ? $('.contacts .mainscreen .contactlist .' + _0x5e9497)
      .show()
      .append(_0x164e4d)
  : $('.contacts .mainscreen .contactlist .other')
      .show()
      .append(_0x164e4d);
...
ESX.Phone.functions.loadEditContactImage = function () {
  const contact = ESX.Phone.currentContact;
  if (contact && contact.photo && contact.photo !== '') {
    $('.contacts > .editcontact #contact-image')
      .attr('src', contact.photo)
      .show();
  } else {
    $('.contacts > .editcontact #contact-image').hide();
  }
};
...
ESX.Phone.functions.addButton('#message-contact2', function () {
  let _0x51e539 = ESX.Phone.chats.find(
    (_0x32d5b3) => _0x32d5b3.number == ESX.Phone.currentContact.number
  )
  if (_0x51e539) {
...
ESX.Phone.functions.addButton('#edit-contact', function () {
  ...
  $('.editcontact input[name="photo"]').on('input', function () {
  const url = $(this).val().trim()
  const img = $('.editcontact img.avatar')

  if (url !== '' && url.match(/\.(jpeg|jpg|gif|png)$/i)) {
    img.attr('src', url)
  } else {
    img.attr('src', 'media/default.png')
  }
})
...
ESX.Phone.functions.updateAvatarPreview = function (photo, name, scope) {
  const imageEl = $(`${scope} #contact-image`);
  const letterEl = $(`${scope} #contact-letter`);

  if (photo && photo.trim() !== '') {
    imageEl.attr('src', photo).show();
    letterEl.hide();
  } else {
    const initials = name
      .split(' ')
...
ESX.Phone.functions.addButton('#createnewcontact', async function () {
  let _0x41b4da = document.forms.newcontact,
    ...
    $('.newcontact input[name="photo"]').on('input', function () {
  const url = $(this).val().trim()
  const img = $('.newcontact img.avatar')
...
ESX.Phone.functions.updateBank = async function (data) {
 ...
 if (!data || typeof data.balance === 'undefined' || typeof data.iban === 'undefined') {
  data = {
    balance: 0,
    iban: "IBAN Yok"
  };
...
// Bu kontrol senin gösterimi engelliyor olabilir, GEÇİCİ OLARAK KALDIR:
if (ESX.Phone.currentApp !== 'Bank') {
  
  // return; // ← bunu geçici olarak yoruma al (veya kaldır)
}


// Veriyi bastır

$('.character-name').html('Merhaba, <strong>' + data.holder + '</strong>');

// USD olan kutuya içerik ekle
$('#bank-balance').html(`
  <span style="display:block;text-align: center; font-size:0.85em; color:white;">USD</span>
  <span style="font-size:1em; font-weight:bold; color:#ffffff;">
    ${
      Config.CurrencyPosition == 2
        ? ESX.Phone.functions.formatNumber(data.balance) + ' ' + Config.Currency
        : Config.Currency + ' ' + ESX.Phone.functions.formatNumber(data.balance)
    }
  </span>
`);

// EUR olan kutuya sabit içerik ekle
$('#bank-balance-eur').html(`
  <span style="display:block;text-align: center; font-size:0.85em; color:white;">EUR</span>
  <span style="font-size:1em; font-weight:bold; color:#ffffff;">€ 0.00</span>
`);


$('#account-title').text(data.iban || "IBAN Yok");
...
_0x39730e.forEach((_0x205b0a) => {
$('.bank .invoicelist').append(
  '<div class="invoice no-icon" data-type="invoice" data-id="' + _0x205b0a.id + '">' +
    '<div class="content">' +
      '<p class="title">' + _0x205b0a.title + '</p>' +
      '<p class="description">' +
    ...
    '</div>' +
    '<div class="buttons">' +
      '<div class="button" data-action="pay" style="--background-color: #7ca663;">Öde</div>' +
...
ESX.Phone.functions.addButton('#transfer', function () {
  ...
  let _0x5106c3 = document.querySelector('input[name="id"]').value,
    _0x202b93 = document.querySelector('input[name="amount"]').value
