...
$(document).ready(function () {
    ...
    window.addEventListener("message", function (event) {
        if (event.data.action === "ui") {
            if (firstOpen) {
                firstOpen = false;
                $.post("https://ak4y-multicharacter/started");
            }
            if (event.data.toggle) {
                ...
                setTimeout(() => {
                    $.post("https://ak4y-multicharacter/setupCharacters");
                    setTimeout(() => {
...
$(document).on("click", "#createButton", function () {
    ...
    if (selectedDivId.id === "createButton") {
        ...
        $.post("https://ak4y-multicharacter/cDataPed");
...
$(document).on("click", ".sexButton", function () {
    ...
    $.post(
        "https://ak4y-multicharacter/cDataPed",
        JSON.stringify({
            sex: sex,
        })
    );
...
$(document).on("click", "#redeemAccept", function () {
    ...
    if (input) {
        firstOpen = true;
        $.post(
            "https://ak4y-multicharacter/sendInput",
            JSON.stringify({
                inputData: input,
            }),
            function (data) {
...
$(document).on("click", "#clickablePlay", function () {
    ...
    $.post(
        "https://ak4y-multicharacter/selectCharacter",
        JSON.stringify({
            cData: cid,
        })
    );
...
$(document).on("click", "#agreeDeleteButton", function () {
    ...
    firstOpen = true;
    $.post(
        "https://ak4y-multicharacter/removeCharacter",
        JSON.stringify({
            citizenid: parseInfo.citizenid,
        })
    );
    $(".agreeDeleteSection").fadeOut(100);
    // $.post("https://ak4y-multicharacter/refreshCharacters");
    // setTimeout(() => {
    // }, 1000);
    // $(".agreeDeleteSection").fadeOut(100);
});
        ...
        if (index + 1 <= myCount) {
            ...
            $(".charListRightArea").append(`
                <div class="charItem locked" id="locked" data-uniqueId="${index + 1}">
                    ...
                    <div class="createCharText">${translate.buySlot}</div>
                </div>
            `);
        }
    }
